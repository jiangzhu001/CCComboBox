//@ts-ignore
import { _decorator, Component, Node, EventTouch, Vec2, Label, Sprite, Tween, tween, Vec3, settings, Sorting2D, RichText, sys } from 'cc';
...
export class VScrollViewItem extends Component {
  ...
  public onSortLayer() {
    ...
    // Sprite 先排（低 orderNumber），Label 后排（高 orderNumber），确保 Label 始终渲染在 Sprite 上方
    sprites.forEach((c, i) => changeUISortingLayer(c.node, 0, i + 1));
    labels.forEach((c, i) => changeUISortingLayer(c.node, 0, sprites.length + i + 1));
  }
  ...
  public offSortLayer() {
    const labels = this.node.getComponentsInChildren(Label);
    const sprites = this.node.getComponentsInChildren(Sprite);
    const nodes = [...labels.map(c => c.node), ...sprites.map(c => c.node)];
    for (const node of nodes) {
      changeUISortingLayer(node, 0, 0);
    }
  }
